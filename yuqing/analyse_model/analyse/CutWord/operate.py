...
def locCutWords(article):
    ...
    :rtype : str
    """
    key_file = open(current_path + '/dict/region_key_words', 'rb').read().decode('utf-8')
    key_word = set((u""))
...
def sentiment_analysis(article):
    ...
    word_file_pos = open(current_path + '/dict/pos_sentiment.txt', 'rb').read().decode('utf-8')
    pos_word = set((u""))
    for pos_file_words in word_file_pos.splitlines():
        pos_word.add(pos_file_words.strip())
